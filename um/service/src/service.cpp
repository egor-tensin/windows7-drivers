...
ServiceHandle open_service(const ServiceManager& mgr, const std::string& name) {
    const auto raw = OpenServiceA(mgr, name.c_str(), SERVICE_ALL_ACCESS);
...
bool service_exists(const ServiceManager& mgr, const std::string& name) {
    const auto raw = OpenServiceA(mgr, name.c_str(), SERVICE_QUERY_STATUS);
...
SERVICE_STATUS_PROCESS query_service_status(const ServiceHandle& handle) {
    ...
    if (!QueryServiceStatusEx(handle, SC_STATUS_PROCESS_INFO, buf_ptr, buf_size, &nbreq)) {
        const auto ec = GetLastError();
        throw windows_error::make(ec, __FILE__, __LINE__, __FUNCTION__);
    }
...
bool Service::exists(const ServiceManager& mgr, const std::string& name) {
